...
export async function PUT(request: NextRequest) {
  ...
  // SECURITY: Prevent overwriting an existing vault.
  // If the user already has a passphrase, they must unlock, not create new.
  ...
  if (existingStatus?.has_passphrase) {
    return NextResponse.json(
      { error: 'Vault already exists. Please unlock with your existing passphrase.' },
      { status: 409 },
    );
  }

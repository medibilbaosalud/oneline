...
const inputCls =
  'mt-1 w-full rounded-xl border border-neutral-800 bg-neutral-900 px-3 py-2 text-sm text-neutral-100 focus:outline-none focus:ring-2 focus:ring-indigo-500';

export default function YearStoryClient() {
  ...
  async function generate() {
    ...
    try {
      ...
      setErr(error instanceof Error ? error.message : 'Failed to generate the story');
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="mx-auto max-w-3xl space-y-6 px-6 py-12">
        ...
        <section className="grid gap-4 rounded-2xl border border-white/10 bg-neutral-900/60 p-5 md:grid-cols-2">
          <div className="space-y-4">
            ...
            <label className="block text-sm">
              Length
              <select className={inputCls} value={length} onChange={(event) => setLength(event.target.value as LengthOption)}>
            ...
            <label className="block text-sm">
              Tone
              <select className={inputCls} value={tone} onChange={(event) => setTone(event.target.value as ToneOption)}>
            ...
            <label className="block text-sm">
              Point of view
              <select className={inputCls} value={pov} onChange={(event) => setPov(event.target.value as PovOption)}>
          ...
          <div className="space-y-4">
            <label className="block text-sm">
              ...
              <select
                className={inputCls}
                value={pinnedWeight}
                onChange={(event) => setPinnedWeight(Number(event.target.value) as 1 | 2 | 3)}
              >

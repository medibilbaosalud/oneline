...
export default function SignUpPage() {
  ...
  const redirectTo = qs?.get("redirectTo");
  const onboardingPath = useMemo(() => {
    return redirectTo
      ? `/onboarding/vault?redirectTo=${encodeURIComponent(redirectTo)}`
      : "/onboarding/vault";
  }, [redirectTo]);
  ...
  const onSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const message = error instanceof Error && error.message ? error.message : "No pudimos enviar el enlace.";
      setErr(message);
    } finally {
      setLoading(false);
    }

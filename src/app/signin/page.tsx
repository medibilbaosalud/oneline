...
export default function SignInPage() {
  ...
  const next = qs?.get("redirectTo") || "/today";
  ...
  const onPasswordSignIn = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const message = error instanceof Error && error.message ? error.message : "No pudimos iniciar sesión.";
      setErr(message);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <main className="mx-auto max-w-md p-6">
      ...
      <p className="text-sm text-muted-foreground mb-6">Usa tu cuenta existente. Si aún no tienes, crea una aquí.</p>

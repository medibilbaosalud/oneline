...
function NavLink({ href, label, onClick }: { href: string; label: string; onClick?: () => void }) {
  ...
  return (
    <Link
      ...
      onClick={onClick}
      className={
        active ? `${base} bg-neutral-800 text-white` : `${base} text-neutral-300 hover:bg-neutral-800/60`
      }
    >
      {label}
    </Link>
    ...
    <header className="sticky top-0 z-50 border-b border-white/10 bg-neutral-950/70 backdrop-blur supports-[backdrop-filter]:bg-neutral-950/60">
      ...
      {open && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          {/* Panel */}
          <div className="absolute right-0 top-0 h-full w-[82%] max-w-sm bg-neutral-950/95 ring-1 ring-white/10">
